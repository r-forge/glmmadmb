...
<<coefplotmg1,fig.width=8,warning=FALSE>>=
library(coefplot2)
...
lme4fun(c(v2,fixef(gm1_lme4))) ## observed variance (*almost* identical)
lme4fun(c(v2,fixef(gm1_glmmADMB))) ## observed variance (*almost* identical)
@ 

<<>>=
glmmadmbfun <- function(
glmmadmb(cbind(incidence, size - incidence) ~ period + (1 | herd),
                  data = cbpp, family = "binomial",
         extra.args=c("maxiter 1"))
@ 
