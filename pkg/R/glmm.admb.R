    ...
    imaxfn = 10, save.dir=NULL,offset)
{
    ...
    dir.create(dirname)
    setwd(dirname)
    if(is.null(save.dir))on.exit(unlink(dirname, recursive = TRUE))
    ...
    dat_list = list(n = n, y = y, p = p, X = X, q = q, m = m,
        ...
        easy_flag = as.numeric(easyFlag), zi_flag = as.numeric(zeroInflation),
    ...
    pin_list = list(pz = ifelse(zeroInflation, 0.02, 1e-04),
        b = numeric(p), tmpL = 0.25 + numeric(m), tmpL1 = 1e-04 +
            numeric(m * (m - 1)/2), logalpha = 2.0, kkludge = 0,
    ...
    if(.Platform$OS.type=="windows"){
      shell(paste(.path.package("glmmADMB"),"/admb/",file_name,".exe",
            " ",cmdoptions, sep=""), invisible = TRUE)
    }else{
      system(paste("cp ",.path.package("glmmADMB"),"/admb/",file_name,
             " .", sep=""))
    ...
    if (!missing(random)) {
        ...
        out$sd_U = matrix(as.numeric(tmp[tmpindex == "u", 4]),
            ncol = m, byrow = TRUE, dimnames = list(data[, group][group_d[-1] -
                1], colnames(Z)))
    }
