...
function (object, ...)
{
    ...
    if (length(unique(paste(lapply(objects, function(x) x$random)))) >
        1) {
    ...
    dimnames(table) <- list(1:length(objects), c("NoPar", "LogLik",
        "Df", "-2logQ", "P.value"))
    title <- "Analysis of Variance Table\n"
    topnote <- paste("Model ", format(1:length(objects)), ": ",
