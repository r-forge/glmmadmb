...
\begin{itemize}
  \item response distributions: Poisson, binomial, negative binomial (NB1 and NB2 parameterizations), Gamma, Beta; Gaussian coming soon
  \item link functions: log, logit, probit, cloglog, inverse, identity
...
\item specify a model for the fixed effects, in the standard
  R (Wilkinson-Rogers) formula notation. Formulae can also
  include \emph{offsets}.
\item specify a model for the random effects, in the notation
  ...
  is an effect (e.g. \code{1} for a random-intercept model
  or \code{time} for a model with random variation in slopes
  through time across \ldots  Random effects can be specified
  either in a separate \code{random} argument (as in \code{nlme})
  or as part of the model formula (as in \code{lme4})
\item specify a family (as a string: e.g. \code{"poisson"}
  or \code{"binomial"})
\item optionally specify a link function,
  as a string (default link functions
  for families are the canonical links (e.g. binomial=\code{"logit"},
  Poisson and negative binomial=\code{"log"}) except that the
  default link for the Gamma in \code{glmmADMB} is \code{"log"}
  not the canonical inverse link
...
gt1 <- system.time(glmmadmb(NCalls~(FoodTreatment+ArrivalTime)*
                                     SexParent+
...
gfit1 <- glmmadmb(NCalls~(FoodTreatment+ArrivalTime)*
                                     SexParent+
                                     offset(logBroodSize)+(1|Nest),
                                     data=Owls,
                                     zeroInflation=TRUE,
...
gfit2 <- glmmadmb(NCalls~(FoodTreatment+ArrivalTime)*SexParent+
                  offset(logBroodSize)+(1|Nest),
                  data=Owls,
                  zeroInflation=TRUE,
                  family="nbinom")
...
gfit4 <- glmmadmb(NCalls~(FoodTreatment+ArrivalTime)*SexParent+
                                     BroodSize+(1|Nest),
                                     data=Owls,
                                     zeroInflation=TRUE,
                                     family="nbinom1")
