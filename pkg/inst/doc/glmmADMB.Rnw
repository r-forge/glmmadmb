...
\begin{itemize}
  \item response distributions: Poisson, binomial, negative binomial (NB1 and NB2 parameterizations), Gamma, Beta, truncated Poisson and negative binomial; Gaussian coming soon
...
chain of 1000 iterations was much too short --- which took about two
hours on a modern (2011) laptop.
...
Look at the trace plots.  (Unfortunately, the MCMC variables
...
<<>>=
plotDens(m)
## very slow and not particularly useful for this large a set of parameters ...
plotSplom(m,pch=".")  
@ 
...
\textbf{Missing}: specifying starting values; MCMC; general
troubleshooting (extra arguments, running outside R)
